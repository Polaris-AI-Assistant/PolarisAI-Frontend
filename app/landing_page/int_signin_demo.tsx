        ...
        <section className="w-full">
            <div className="py-12">
                <div className="mx-auto max-w-5xl px-6">
                    ...
                    <div className="relative z-20 mx-auto mb-20 max-w-lg space-y-4 text-center">
                        <h2 className={`${spaceGrotesk.className} text-balance text-4xl md:text-5xl font-bold text-white tracking-tight`}>
                            Integrate with your favorite tools
                        </h2>
                        <p className="text-gray-400">Connect seamlessly with popular platforms and services to enhance your workflow.</p>
                    </div>
                    
                    {/* Circle visualization - now at bottom */}
                    <div className="aspect-16/10 group relative mx-auto flex max-w-[22rem] items-center justify-between sm:max-w-sm">
                        ...
                        <div className="bg-linear-to-b from-muted-foreground/15 absolute inset-0 flex aspect-square items-center justify-center rounded-full border-t to-transparent to-25%">
                            <IntegrationCard className="-translate-x-1/6 absolute left-0 top-1/4 -translate-y-1/4">
                                <Image src="/Google_Calendar_icon_(2020).svg.png" alt="Google Calendar" width={40} height={40} className="object-contain" />
                            </IntegrationCard>
                            <IntegrationCard className="absolute top-0 -translate-y-1/2">
                                <Image src="/Google_Docs_logo_(2014-2020).svg.png" alt="Google Docs" width={40} height={40} className="object-contain" />
                            </IntegrationCard>
                            <IntegrationCard className="translate-x-1/6 absolute right-0 top-1/4 -translate-y-1/4">
                                <Image src="/Google_Forms_2020_Logo.svg.png" alt="Google Forms" width={40} height={40} className="object-contain" />
                            </IntegrationCard>
                        </div>
                        <div className="bg-linear-to-b from-muted-foreground/15 absolute inset-16 flex aspect-square scale-90 items-center justify-center rounded-full border-t to-transparent to-25%">
                            <IntegrationCard className="absolute top-0 -translate-y-1/2">
                                <Image src="/Google_Sheets_logo_(2014-2020).svg.png" alt="Google Sheets" width={40} height={40} className="object-contain" />
                            </IntegrationCard>
                            <IntegrationCard className="absolute left-0 top-1/4 -translate-x-1/4 -translate-y-1/4">
                                <Image src="/Google_Meet-Logo.wine.png" alt="Google Meet" width={150} height={100} className="object-contain" />
                            </IntegrationCard>
                            <IntegrationCard className="absolute right-0 top-1/4 -translate-y-1/4 translate-x-1/4">
                                <Image src="/git3.png" alt="GitHub" width={100} height={100} className="object-contain" />
                            </IntegrationCard>
                        </div>
                        <div className="absolute inset-x-0 bottom-0 mx-auto my-2 flex w-fit justify-center gap-2">
                            <div className="relative z-20">
                                <IntegrationCard isCenter={true}>
                                    <Image src="/polaris.png" alt="Polaris" width={48} height={48} className="object-contain" />
                                </IntegrationCard>
                            </div>
                        </div>
                    </div>
...
const IntegrationCard = ({ children, className, isCenter = false }: { children: React.ReactNode; className?: string; isCenter?: boolean }) => {
    return (
        <div className={cn('relative z-30 flex rounded-full', isCenter ? 'size-20' : 'size-16', className)}>
            <div className={cn('m-auto size-fit', isCenter ? '*:size-12' : '*:size-10')}>{children}</div>
        </div>
    )
}
