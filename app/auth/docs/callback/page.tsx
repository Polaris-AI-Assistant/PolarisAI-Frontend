...
function DocsCallbackContent() {
  ...
  useEffect(() => {
    ...
    if (error) {
      ...
      router.replace(`/dashboard?connectError=docs&connectErrorMsg=${encodeURIComponent(msg)}`);
      return;
    }

    if (success === 'true') {
      localStorage.setItem('docs_connected', 'true');
      router.replace('/dashboard?connectSuccess=docs');
      return;
    }

    router.replace(`/dashboard?connectError=docs&connectErrorMsg=${encodeURIComponent('Unexpected response from Docs OAuth')}`);
  }, [searchParams, router]);
