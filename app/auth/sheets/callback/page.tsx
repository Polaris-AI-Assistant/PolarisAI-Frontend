...
function SheetsCallbackContent() {
  ...
  useEffect(() => {
    ...
    if (error) {
      ...
      router.replace(`/dashboard?connectError=sheets&connectErrorMsg=${encodeURIComponent(msg)}`);
      return;
    }

    if (success === 'true') {
      localStorage.setItem('sheets_connected', 'true');
      router.replace('/dashboard?connectSuccess=sheets');
      return;
    }

    router.replace(`/dashboard?connectError=sheets&connectErrorMsg=${encodeURIComponent('Unexpected response from Sheets OAuth')}`);
  }, [searchParams, router]);
